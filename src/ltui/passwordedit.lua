...
-- define module
local passwordedit = passwordedit or textarea()

-- init textedit
function passwordedit:init(name, bounds, text)
...
-- draw textedit
function passwordedit:on_draw(transparent)
    ...
    if strs and #strs > 0 and textattr then
        local str2 = string.rep("*",string.len(strs[1]))
        self:canvas():attr(textattr):move(0, 0):putstrs({str2}, self._STARTLINE + 1)
    end
...
-- set text
function passwordedit:text_set(text)
...
-- on event
function passwordedit:on_event(e)
